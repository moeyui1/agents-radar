...
type LlmHandler = (prompt: string, maxTokens: number) => Promise<string>;

interface CallLlmOptions {
  provider?: LlmProvider;
  handlers?: Record<LlmProvider, LlmHandler>;
  maxRetries?: number;
  retryBaseMs?: number;
...
export function createCallLlm(options: CallLlmOptions = {}) {
  const providerName = options.provider ?? LLM_PROVIDER;
  ...
  return async function callLlmWithOptions(prompt: string, maxTokens = 4096): Promise<string> {
    const handler = handlers
      ? handlers[providerName]
      : PROVIDERS[providerName].call;
    if (!handler) {
      throw new Error(`No handler configured for provider: ${providerName}`);
    }
